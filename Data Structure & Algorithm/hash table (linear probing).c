...
// lp: Linear Probing (선형 탐사)
void hash_lp_add(element item, element ht[]) // 테이블에 키를 삽입하는 함수
{
	...
	while (!empty(ht[i]))	// 해당 주소(버켓)에 값이 이미 존재하면 선형탐사 실행
	{
		...
		i = (i + 1) % TABLE_SIZE;	// 다음 인덱스의 버켓으로 이동
...
void hash_lp_search(element item, element ht[])
{
	...
	while (!empty(ht[i]))	// 선형 탐사 중 비어있는 버켓을 만나면 탐색 실패
	{
...
void hash_lp_print(element ht[])	// 해시 테이블 출력
{
	int i = 0;
