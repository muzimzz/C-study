...
// qp: Quadratic Probing (이차조사)
void hash_qp_add(element item, element ht[]) // 테이블에 키를 삽입하는 함수
{
	int i = 0, hash_value = 0, inc = 0;

	hash_value = i = hash_function(item.key);
	printf("%s의 hash_address = %d\n", item.key, i);

	while (!empty(ht[i]))	// 해당 주소(버켓)에 값이 이미 존재하면 이차조사 실행
	{
		if (equal(item, ht[i]))
		{
		...
		inc++;;
		i = (i + inc * inc) % TABLE_SIZE;	// 다음 추가할 인덱스의 버켓으로 이동
...
void hash_qp_search(element item, element ht[])
{
	...
	while (!empty(ht[i]))	// 이차조사 중 비어있는 버켓을 만나면 탐색 실패
	{
		...
		inc++;
		i = (i + inc * inc) % TABLE_SIZE;	// 다음 조사할 버켓 인덱스 탐색
...
void hash_qp_print(element ht[])	// 해시 테이블 출력
{
	int i = 0;
